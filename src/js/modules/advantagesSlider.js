const advantagesSlider = () => {
  const slides = document.querySelectorAll('.advantages__row > .advantages__item'),
        dots = () => document.querySelectorAll('.advantages > .container > .slick-dots > ul > li');
  let index = 0;
  ...
  const createDots = () => {
    const dotsParent = document.querySelector('.advantages > .container > .slick-dots > ul');
    ...
    if(arraySlides.length === 0) {
      ...
      const dotsParentWrapper = dotsParent.closest('.slick-dots');
