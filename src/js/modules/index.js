...
const menuInit = () => {
  document.addEventListener('click', ({ target }) => {
    if (target.closest('.navbar-toggler')) {
      const menuBurger = document.querySelector('.navbar-collapse');
      menuBurger.classList.toggle('collapsed');
      // toggleBodyLock(menuBurger.classList.contains('menu-open'))
    }
  })
}
...
// Операции при разных размерах ширины экрана

window.addEventListener('resize', () => {
  const windowWidth = window.innerWidth;

  // Скрытие открытого гамбургера меню.
  try{
    if (windowWidth > 991.98) {
      const element = document.querySelector('.navbar > .navbar-collapse');
